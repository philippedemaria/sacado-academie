...
def print_facture(request,fid):  
    ...
    #########################################################################################
    elements.append(Spacer(0,0.1*inch))
    details_tot = Table([("Hors taxe", str( round(float(total_amount)/1.2,2) )+" €" ),( "TVA 20%", str( round( float(total_amount)-round(float(total_amount)/1.2,2),2)) +" €" ), ("Total TTC", str( round( float(total_amount),2 )) +" €" ) ], hAlign='RIGHT', colWidths=[2.8*inch,1*inch])
