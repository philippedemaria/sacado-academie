...
def print_invoice(request, idi ):
    ...
    #########################################################################################
    elements.append(Spacer(0,0.1*inch))
    details_tot = Table([("Hors taxe", str( round(float(invoice.amount)/1.2,2) )+" €" ),( "TVA 20%", str( round( float(invoice.amount)-float(invoice.amount)/1.2),2) +"0 €" ), ("Total TTC", str( invoice.amount) +" €" ) ], hAlign='RIGHT', colWidths=[2.8*inch,1*inch])
    details_tot.setStyle(TableStyle([
               ('INNERGRID', (0,0), (-1,-1), 0.25, colors.gray),
               ('BOX', (0,0), (-1,-1), 0.25, colors.gray),
                ('BACKGROUND', (0,0), (-1,0), colors.Color(0.9,0.9,0.9))
